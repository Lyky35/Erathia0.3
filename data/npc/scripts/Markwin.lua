...
local function greetCallback(cid)
	...
	if player:getStorageValue(2017) < 1 then
		npcHandler:setMessage(MESSAGE_GREET, "Intruder! Guards, take him down!")
		player:setStorageValue(2017, 1)
		local position
		for x = -1, 1 do
			for y = -1, 1 do
		...
		return false
	elseif player:getStorageValue(2017) == 1 then
		...
		player:setStorageValue(2017, 2)
	elseif player:getStorageValue(2017) == 2 then
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "letter") then
		if player:getStorageValue(2014) == 1 then
