...
local function creatureSayCallback(cid, type, msg)
	...
	local missionProgress = player:getStorageValue(1091)
	if msgcontains(msg, 'spy report') or msgcontains(msg, 'mission') then
		if player:getStorageValue(1090) ~= 2 then
	...
	elseif npcHandler.topic[cid] == 1 then
		if msgcontains(msg, 'yes') then
			if player:getStorageValue(1092) ~= 2 or not player:removeItem(2345, 1) then
				npcHandler:say({
