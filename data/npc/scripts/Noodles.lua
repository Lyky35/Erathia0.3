...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "banana skin") then
		if player:getStorageValue(2010) == 7 then
	...
	elseif msgcontains(msg, "dirty fur") then
		if player:getStorageValue(2010) == 8 then
	...
	elseif msgcontains(msg, "mouldy cheese") then
		if player:getStorageValue(2010) == 9 then
	...
	elseif msgcontains(msg, "like") then
		if npcHandler.topic[cid] == 1  then
			...
			player:setStorageValue(2010, 8)
			npcHandler.topic[cid] = 0
		elseif npcHandler.topic[cid] == 2 then
			npcHandler:say("Woof!", cid)
			player:setStorageValue(2010, 9)
			npcHandler.topic[cid] = 0
		elseif npcHandler.topic[cid] == 3 then
			npcHandler:say("Meeep! Grrrrr! <spits>", cid)
			player:setStorageValue(2010, 10)
			npcHandler.topic[cid] = 0
		end
	end
	return true
