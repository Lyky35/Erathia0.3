...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'morrin') then
		npc:say("Ah, I remember that man. We made a deal, guess about what.", TALKTYPE_SAY)
		npcHandler.topic[cid] = 1
	elseif msgcontains(msg, 'key') and npcHandler.topic[cid] == 1 then
		npc:say("Right! We can make the same deal if you give a fresh delicious roll. Do you have any?", TALKTYPE_SAY)
		npcHandler.topic[cid] = 2
	elseif hagor_talk_state == 2 and msgcontains(msg, 'yes') or hagor_talk_state == 1 and  msgcontains(msg, 'Yes') then
		if player:removeItem(Cfroll, 1) then
			npc:say("Oh, fine! Here you are.", TALKTYPE_SAY)
			local key = player:addItem(2088, 1)
			key:setActionId(4022)
			npcHandler.topic[cid] = 0
		else
			npc:say("Hey, you do not have one!", TALKTYPE_SAY)
			npcHandler.topic[cid] = 0
		end
	elseif msgcontains(msg, 'no') and npcHandler.topic[cid] >= 1 then
		npc:say("Maybe next time.", TALKTYPE_SAY)
		npcHandler.topic[cid] = 0
	end
	return true
end
