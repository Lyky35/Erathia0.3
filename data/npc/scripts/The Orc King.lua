...
local function greetCallback(cid)
	...
	if player:getStorageValue(1095) ~= 1 then
		player:setStorageValue(1095, 1)
...
local function creatureSayCallback(cid, type, msg)
	...
	local player = Player(cid)
	local efreet, marid = player:getStorageValue(1099), player:getStorageValue(1093)
	-- Mission 3 - Orc Fortress
	if msgcontains(msg, 'lamp') then
		if efreet == 1 or marid == 1 then
			if player:getStorageValue(1096) ~= 1 then
				npcHandler:say({
	...
	elseif npcHandler.topic[cid] == 1 then
		if msgcontains(msg, 'malor') then
			if efreet == 1 then
				player:setStorageValue(2003, 1)
			elseif marid == 1 then
				player:setStorageValue(2001, 1)
			end

			player:setStorageValue(1096, 1)
...
keywordHandler:addKeyword({'malor'}, StdModule.say, {npcHandler = npcHandler, text = 'This cursed djinn king! I set him free from an enchanted lamp, and he cheated me!'}, function(player) return player:getStorageValue(1096) == 1 end)
