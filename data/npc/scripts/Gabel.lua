...
local function creatureSayCallback(cid, type, msg)
	...
	local missionProgress = player:getStorageValue(1093)
	if msgcontains(msg, 'mission') then
		if player:getStorageValue(1091) ~= 2 then
			npcHandler:say({
	...
	elseif npcHandler.topic[cid] == 2 then
		if msgcontains(msg, 'yes') then
			...
			player:setStorageValue(1093, 3)
			player:setStorageValue(1094, 1)
