...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "advancement") then
		local storage = player:getStorageValue(1064)
		if storage == 1 then
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 3 then
			if player:removeItem(6500, 500) then
				...
				player:setStorageValue(1064, 2)
				npcHandler:say("You advanced to {Hyaena} rank! You are now able to use teleports of third floor of Knightwatch Tower.", cid)
			else
