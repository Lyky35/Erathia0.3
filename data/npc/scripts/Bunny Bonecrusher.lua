...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'job') then
		if player:getSex() == PLAYERSEX_MALE then
			npc:say("I am the general of the queen's army and have not the time to explain this concept to you.", TALKTYPE_SAY)
		else
			npc:say("I am the general of the queen's army! You really should consider to join, sister.", TALKTYPE_SAY)
		end
	elseif msgcontains(msg, 'quest') and (npcHandler:isFocused(cid)) or msgcontains(msg, 'mission') and (npcHandler:isFocused(cid)) then
		if player:getSex() == PLAYERSEX_MALE then
			npc:say("Yeah. Entrusting a male with an important quest. Get serious!", TALKTYPE_SAY)
		else
			npc:say("Sometimes the queen calls for heroines. Keep eyes and ears open!", TALKTYPE_SAY)
		end
	elseif msgcontains(msg, 'sell') and (npcHandler:isFocused(cid)) then
		npc:say("Are you suggesting I am corruptible?", TALKTYPE_SAY)
	...
	elseif msgcontains(msg, 'no') and npcHandler.topic[cid] == 1 then
		npc:say("You should be careful with your words!", TALKTYPE_SAY)
		npcHandler.topic[cid] = 0
	end
	return true
end
